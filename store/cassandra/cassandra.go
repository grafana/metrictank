...
func PrepareChunkData(span uint32, data []byte) []byte {
	...
	if !ok {
		...
		log.WithFields(log.Fields{
			"span": span,
		}).Panic("cassandra_store: chunk span invalid")
	}
...
func ConvertTimeout(timeout string, defaultUnit time.Duration) time.Duration {
	...
	if err != nil {
		log.WithFields(log.Fields{
			"duration": timeout,
		}).Fatal("cassandra_store: invalid duration value")
	}
	return timeoutD
}
...
func NewCassandraStore(config *StoreConfig, ttls []uint32) (*CassandraStore, error) {
	...
	if err != nil {
		log.WithFields(log.Fields{
			"error": err.Error(),
		}).Error("cassandra_store: failed to create cassandra session")
		return nil, err
	}
	...
	if config.CreateKeyspace {
		...
		}).Info("cassandra_store: ensuring that keyspace exists")
		...
		for _, table := range ttlTables {
			log.WithFields(log.Fields{
				"table": table.Name,
			}).Info("cassandra_store: ensuring that table exists")
	...
	AttemptLoop:
		for attempt := 1; attempt > 0; attempt++ {
			...
			if err != nil {
				log.WithFields(log.Fields{
					"attempt": attempt,
				}).Warn("cassandra_store: keyspace not found")
				if attempt >= 5 {
					return nil, err
				}
				...
				for _, table := range ttlTables {
					if _, ok := keyspaceMetadata.Tables[table.Name]; !ok {
						log.WithFields(log.Fields{
							"table":   table.Name,
							"attempt": attempt,
						}).Warn("cassandra_store: table not found")
						if attempt >= 5 {
							return nil, err
						}
						time.Sleep(5 * time.Second)
	...
	log.WithFields(log.Fields{
		"config": config,
	}).Debug("cassandra_store: created session with config")
	c := &CassandraStore{
		Session:          session,
...
func (c *CassandraStore) processWriteQueue(queue chan *mdata.ChunkWriteRequest, meter *stats.Range32) {
	...
	for {
		...
		case cwr := <-queue:
			...
			log.WithFields(log.Fields{
				"key":      cwr.Key,
				"chunk.t0": cwr.Chunk.T0,
				"chunk":    cwr.Chunk,
			}).Debug("cassandra_store: starting to save")
			...
			for !success {
				...
				if err == nil {
					...
					log.WithFields(log.Fields{
						...
						"chunk.t0": cwr.Chunk.T0,
						"chunk":    cwr.Chunk,
					}).Debug("cassandra_store: save complete")
					chunkSaveOk.Inc()
				} else {
					errmetrics.Inc(err)
					if (attempts % 20) == 0 {
						log.WithFields(log.Fields{
							...
							"chunk":    cwr.Chunk,
							"error":    err.Error(),
						}).Warn("cassandra_store: failed to save chunk to cassandra")
					}
					chunkSaveFail.Inc()
