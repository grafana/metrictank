...
func (i *idSelector) byTagValueFromMetaTagIndex() {
	...
	for _, recordId := range i.ctx.metaTagIndex.tags.getByTagValue(i.expr, false) {
		select {
		case <-i.stopCh:
			return
		default:
...
func (i *idSelector) byTagFromMetaTagIndex() {
	...
	for _, recordId := range i.ctx.metaTagIndex.tags.getByTag(i.expr, false) {
		select {
		case <-i.stopCh:
			return
		default:
...
func (i *idSelector) evaluateMetaRecord(id recordId) {
	record, ok := i.ctx.metaTagIndex.records.getMetaRecordById(id)
	if !ok {
		return
	}
...
func (i *idSelector) runSubQuery(query TagQueryContext) {
	defer i.workerWg.Done()
	query.Run(i.ctx.index, i.ctx.byId, nil, i.rawResCh)
	<-i.concGate
}
