...
func init() {
	...
	fs.IntVar(&clientConf.BatchNumMessages, "batch-num-messages", 100, "Maximum number of messages batched in one MessageSet")
	fs.IntVar(&clientConf.ConsumerThreads, "consumer-threads", 1, "Number of kafka client threads")
	fs.IntVar(&clientConf.EventsChannelSize, "events-channel-size", 5, "Size of the channel that enqueues incoming events")
	...
	fs.IntVar(&clientConf.ChannelBufferSize, "channel-buffer-size", 100, "Maximum number of messages allowed on the producer queue")
