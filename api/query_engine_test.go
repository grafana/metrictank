...
func TestAlignRequestsLikeGraphite(t *testing.T) {
	input := []alignCase2{
		{
	...
	for i, ac := range input {
		out, err := alignRequestsLikeGraphite(ac.now, ac.reqs, ac.aggSettings)
		if err != ac.outErr {
			t.Errorf("different err value for testcase %d  expected: %v, got: %v", i, ac.outErr, err)
		}
		if len(out) != len(ac.outReqs) {
...
func BenchmarkAlignRequestsLikeGraphite(b *testing.B) {
	var res []models.Req
	reqs := []models.Req{
		reqRaw("a", 0, 3600*24*7, 1000, 10, consolidation.Avg),
		reqRaw("b", 0, 3600*24*7, 1000, 30, consolidation.Avg),
	...
	for n := 0; n < b.N; n++ {
		res, _ = alignRequestsLikeGraphite(14*24*3600, reqs, aggSettings)
	}
	result = res
}
