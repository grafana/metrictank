...
Grafana Metrictank aims to be a drop-in replacement for Graphite, but also to address a few of Graphite's shortcomings.
...
* many functions are not implemented yet in Grafana Metrictank itself, but it autodetects this and will proxy requests it cannot handle to graphite-web
  (which then uses Grafana Metrictank as a simple backend).  See below for details



## Processing functions

Grafana Metrictank aims to be able to provide as much processing power as it can: we're in the process
of implementing [Graphite's extensive processing api](http://graphite.readthedocs.io/en/latest/functions.html) into Grafana Metrictank itself.

Below is an overview of all current Graphite functions (as of Aug 15, 2018) and their support in Grafana Metrictank.
There are 3 levels of support:

* No : not implemented yet in Grafana Metrictank or not applicable (e.g. graphite functions that affect graphical style but not json data)
* Stable : 100% compatible with graphite and vetted
* Unstable: not fully compatible yet or not vetted enough

When you request functions that Grafana Metrictank cannot provide, it will automatically proxy requests to graphite for a seamless failover.
...
| Function name and signature                                    | Alias        | Grafana Metrictank |
| -------------------------------------------------------------- | ------------ | ------------------ |
| absolute                                                       |              | Stable             |
| aggregate                                                      |              | No                 |
| aggregateLine                                                  |              | No                 |
