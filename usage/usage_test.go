...
// we won't use this
func (f *FakeAggMetric) Get(from, to uint32) mdata.Result {
	return mdata.Result{Oldest: 0, Iters: make([]chunk.Iter, 0)}
}
func (f *FakeAggMetric) GetAggregated(consolidator consolidation.Consolidator, aggSpan, from, to uint32) mdata.Result {
	return mdata.Result{Oldest: 0, Iters: make([]chunk.Iter, 0)}
}
