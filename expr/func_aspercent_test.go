...
var b1AsPercentOfb2 = []schema.Point{
	{Val: -1, Ts: 10},
	{Val: -10, Ts: 20},
	{Val: 100 * (math.MaxFloat64 / float64(1000)) / math.MaxFloat64, Ts: 30},
	{Val: math.NaN(), Ts: 40},
}
...
func TestAsPercentMultiInUsingSelf(t *testing.T) {
	f := getNewAsPercent(
		[][]models.Series{
			{
				{
	...
	out := []models.Series{
		{
			...
			QueryPatt:  "asPercent(a)",
			Target:     "asPercent(a)",
			Datapoints: getCopy(a1AsPercentOfa1b1b1),
		},
		{
			Interval:   10,
			QueryPatt:  "asPercent(foo.*)",
			Target:     "asPercent(foo.a)",
			Datapoints: getCopy(b1AsPercentOfa1b1b1),
		},
		{
			Interval:   10,
			QueryPatt:  "asPercent(foo.*)",
			Target:     "asPercent(foo.b)",
			Datapoints: getCopy(b1AsPercentOfa1b1b1),
		},
	}
	got, err := f.Exec(make(map[Req][]models.Series))
